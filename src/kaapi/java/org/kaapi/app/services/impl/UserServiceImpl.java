...
public class UserServiceImpl implements UserService {
	...
	public User webLogin(FrmWebLogin wFrm) {
		String sql = " SELECT  u.userid, u.email, u.password, u.username, u.gender, u.dateofbirth, u.phonenumber,u.registerdate,u.userimageurl, u.universityid , uni.universityname, u.departmentid ,dep.departmentname , u.point , co.coverphoto as coverphotourl, u.userstatus,u.isconfirmed, "
				...
				+ " LEFT JOIN TBLVIDEO v ON u.USERID=v.USERID " + " LEFT JOIN TBLCOMMENT c ON u.USERID=c.USERID"
	...
	public List<User> listUser(Pagination pagination) {
		String sql = " SELECT  u.userid, u.email, u.password, u.username, u.gender, u.dateofbirth, u.phonenumber,u.registerdate,u.userimageurl, u.universityid , uni.universityname, u.departmentid ,dep.departmentname, u.point, co.coverphoto as coverphotourl,"
				...
				+ " LEFT JOIN tblcoverphoto co ON u.userid = co.userid "
				+ " LEFT JOIN tbluniversity uni ON u.universityid = uni.universityid"
				+ " LEFT JOIN tbldepartment dep ON u.departmentid = dep.departmentid" + " WHERE u.userstatus = '1'"
	...
	public List<User> searchUserByUsername(User user, Pagination pagination) {
		...
		String userType = "";
		if (!user.getUserTypeId().trim().equals("")) {
			userType = " AND u.usertypeid =" + Encryption.decode(user.getUserTypeId());
		}

		String scType = "";
		if (!user.getScType().trim().equals("")) {
			scType = " AND u.usertypeid =" + user.getScType();
		}

		String getUserId = "";
		if (!user.getUserId().trim().equals("")) {
			getUserId = " AND u.isconfirmed =" + user.getUserId();
		}
		...
		String sql = " SELECT"
				...
				+ " ut.usertypeid, ut.usertypename  ,  u.sc_fb_id , u.isconfirmed , u.sc_type, u.resend_count, u.signup_with "
				+ " FROM" + " TBLUSER u INNER JOIN TBLUSERTYPE ut ON u.USERTYPEID=ut.USERTYPEID"
				+ " LEFT JOIN tbluniversity uni ON u.universityid = uni.universityid"
				+ " LEFT JOIN tbldepartment dep ON u.departmentid = dep.departmentid" + " WHERE"
				+ " LOWER(u.username) LIKE LOWER(?) " + " AND LOWER(u.email) LIKE LOWER(?) "

				+ userType + " "// + " AND CAST(u.usertypeid AS VARCHAR) LIKE ?"
				+ scType + " "// " AND CAST(u.sc_type AS VARCHAR ) LIKE ?"
				+ getUserId + " " // " AND CAST(u.isconfirmed AS VARCHAR) LIKE
									// ?"

				+ scFbId + " " + between + " " + " ORDER BY u.userid DESC  offset ? limit ?"; // GROUP
																								// BY
																								// u.USERID,
																								// ut.USERTYPEID,
																								// uni.universityid
		...
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql)) {
			...
			while (rs.next()) {
				u = new User();
				u.setScTwitterId(rs.getString("signup_with")); // Sign up with
				u.setUserId(Encryption.encode(rs.getString("userid")));
				u.setUsername(rs.getString("username"));
				u.setEmail(rs.getString("email"));
				u.setGender(rs.getString("gender"));
	...
	public int countSearchUserByUsername(User user) {
		...
		System.out.println("user.getUserId() ==>" + user.getUserId().trim());
		...
		String sql = " SELECT" + " COUNT(u.userid) " + " FROM"
		...
		// user.getEmail() + " | " + user.getUserTypeId() + " " +
		// user.getScType() + " | " + user.isConfirmed() );
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql);) {
			ps.setString(1, "%" + user.getUsername() + "%");
			ps.setString(2, "%" + user.getEmail() + "%");
			ResultSet rs = ps.executeQuery();
	...
	public User getUSerById(String id) {
		String sql = " SELECT  u.userid, u.email, u.password, u.username, u.gender, u.dateofbirth, u.phonenumber,u.registerdate,u.userimageurl, u.universityid , uni.universityname, u.departmentid ,dep.departmentname , u.point, co.coverphoto as coverphotourl,"
				...
				+ " LEFT JOIN tblcoverphoto co ON u.userid = co.userid"
				+ " LEFT JOIN tbluniversity uni ON u.universityid = uni.universityid"
				+ " LEFT JOIN tbldepartment dep ON u.departmentid = dep.departmentid" + " WHERE u.userid = ?"
	...
	public boolean insertUser(FrmAddUser user) {
		String sql = " INSERT INTO TBLUSER"
				...
				+ " VALUES" + " (NEXTVAL('seq_user'),?,?,?,?,NOW(),'" + environment.getProperty("KA.path")
				+ "/resources/upload/file/user/avatar.jpg',2,?,?,'1',false,? , ?);";
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql)) {
	...
	public boolean isUpdateUserFaceboook(FrmAddUser user) {
		...
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql)) {
			...
			/*
			 * if(user.getDateofbirth() == null){ ps.setDate(4, null); }else{
			 * ps.setDate(4, new
			 * java.sql.Date(user.getDateofbirth().getTime())); }
			 */
			if (ps.executeUpdate() > 0)
				return true;
		} catch (SQLException e) {
	...
	public boolean mobileInsertUser(FrmMobileRegister user) {
		String sql = " INSERT INTO TBLUSER"
				...
				+ " VALUES" + " (NEXTVAL('seq_user'),?,?,?,?,NOW(),?,2,?,?,'1',false,?,?,?,?,?,?,1);";
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql)) {
			ps.setString(1, user.getEmail());
			ps.setString(2, user.getPassword());
			ps.setString(3, user.getUsername());
	...
	public User getUSerEmail(String email) {
		...
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql)) {
			...
			if (rs.next()) {
				// System.err.println(rs.getInt("userid") + "/ Crush " +
				// rs.getString("email") );
				User u = new User();
				u.setUserId(Encryption.encode(rs.getString("userid")));
				u.setEmail(email);
	...
	public boolean checkSocialID(String scType, String scID) {
		...
		// String sqlTW = "select id, count(sc_fb_id) WHERE sc_fb_id = ? and
		// sc_type=?";
		// String sqlGM = "select id, count(sc_fb_id) WHERE sc_fb_id = ? and
		// sc_type=?";
	...
	public boolean insertUserSC(FrmAddUser u) {
		String sqlFB = " INSERT INTO TBLUSER"
				...
				+ " VALUES" + " (NEXTVAL('seq_user'),?,?,?,?,NOW(),?,2,?,?,'1',true,?,?,?,?);";
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sqlFB)) {
	...
	public User findUserById(String userId) {
		String sql = " SELECT  u.userid, u.email, u.password, u.username, u.gender, u.dateofbirth, u.phonenumber,u.registerdate,u.userimageurl, u.universityid , uni.universityname, u.departmentid ,dep.departmentname , u.point , co.coverphoto as coverphotourl, u.userstatus,u.isconfirmed, "
				...
				+ " LEFT JOIN tbluniversity uni ON u.universityid = uni.universityid"
				+ " LEFT JOIN tbldepartment dep ON u.departmentid = dep.departmentid"
				+ " WHERE u.userid=?  AND u.userstatus = '1'"
				+ " GROUP BY u.USERID, ut.USERTYPEID, uni.universityid , dep.departmentid ,co.coverid";
		try (Connection cnn = dataSource.getConnection(); PreparedStatement ps = cnn.prepareStatement(sql);) {
